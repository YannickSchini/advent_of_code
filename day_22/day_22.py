...
def part_1(file_path: str) -> int:
    ...
    number_of_turns, winning_deck = play_one_game_part_1(
        player_1_deck, player_2_deck)
    return calculate_score(winning_deck)


def play_one_turn_part_1(
...
def play_one_game_part_1(player_1_deck: List[int],
                         player_2_deck: List[int]) -> Tuple[int, List[int]]:
    turn_counter = 0
    while len(player_1_deck) > 0 and len(player_2_deck) > 0:
        player_1_deck, player_2_deck = play_one_turn_part_1(
            player_1_deck, player_2_deck)
        turn_counter += 1
...
def part_2(file_path: str) -> int:
    player_1_deck, player_2_deck = parse_inputs(file_path)
    winner, winning_deck = play_one_game_part_2(player_1_deck, player_2_deck)
    return calculate_score(winning_deck)


def play_one_turn_part_2(
        player_1_deck: List[int],
        player_2_deck: List[int]) -> Tuple[List[int], List[int]]:
    new_player_1_deck = player_1_deck.copy()
    new_player_2_deck = player_2_deck.copy()
    player_1_card = new_player_1_deck.pop(0)
    player_2_card = new_player_2_deck.pop(0)
    if player_1_card < len(player_1_deck) and player_2_card < len(
            player_2_deck):
        winner, _ = play_one_game_part_2(new_player_1_deck[:player_1_card],
                                         new_player_2_deck[:player_2_card])
        if winner == 0:
            new_player_1_deck.extend([player_1_card, player_2_card])
        elif winner == 1:
            new_player_2_deck.extend([player_2_card, player_1_card])
...
def play_one_game_part_2(player_1_deck: List[int],
                         player_2_deck: List[int]) -> Tuple[int, List[int]]:
    previous_game_states = [(player_1_deck, player_2_deck)]
    while len(player_1_deck) > 0 and len(player_2_deck) > 0:
        player_1_deck, player_2_deck = play_one_turn_part_2(
            player_1_deck, player_2_deck)
        if (player_1_deck, player_2_deck) in previous_game_states:
            return 0, player_1_deck
        else:
            previous_game_states.append((player_1_deck, player_2_deck))
    winner = int(len(player_1_deck) == 0)
    return winner, player_1_deck + player_2_deck
